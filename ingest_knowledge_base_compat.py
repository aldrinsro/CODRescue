"""
Script d'ingestion de données en mode compatibilité (SANS pgvector natif)
Stocke les embeddings en JSONB au lieu de vector
"""
...
def insert_into_knowledge_base(content: str, metadata: Dict, embedding: List[float]):
    ...
    Args:
        ...
        embedding: Vecteur d'embedding (sera stocké en JSONB)
    """
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Ingestion de données (mode compatibilité)")
    ...
    print("🚀 Ingestion de données (mode compatibilité JSONB)")
    print("=" * 60)
    
    # Vérifier la clé API OpenAI
    if not os.getenv('OPENAI_API_KEY') and not args.stats:

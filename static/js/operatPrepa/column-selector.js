// Sélecteur de colonnes pour l'interface des opérateurs de préparation
(function() {
  const STORAGE_KEY_PREFIX = 'yz.operatprepa.columns.visibility';
  ...
  function saveHidden(indices) {
    try { 
      localStorage.setItem(getStorageKey(), JSON.stringify(indices || [])); 
    } catch {}
  }

  function ensurePanel() {
    let panel = document.getElementById('yzOperatPrepaColumnsPanel');
    if (panel) return panel;
    panel = document.createElement('div');
    panel.id = 'yzOperatPrepaColumnsPanel';
    ...
    panel.innerHTML = `
      <div style="padding:12px 16px; border-bottom:1px solid #e5e7eb; display:flex; align-items:center; justify-content:space-between;">
        ...
        <button id="yzOperatPrepaColumnsClose" style="color:#6b7280; background:none; border:none; cursor:pointer; font-size:18px;">✕</button>
      </div>
      <div id="yzOperatPrepaColumnsBody" style="padding:12px; overflow:auto; max-height:48vh;"></div>
      <div style="padding:12px; border-top:1px solid #e5e7eb; display:flex; gap:8px; justify-content:flex-end;">
        <button id="yzOperatPrepaColumnsReset" style="padding:8px 12px; border:1px solid #d1d5db; border-radius:8px; color:#374151; background:#fff; cursor:pointer;">Réinitialiser</button>
        <button id="yzOperatPrepaColumnsApply" style="padding:8px 12px; border-radius:8px; color:#fff; background: var(--preparation-primary); cursor:pointer;">Appliquer</button>
      </div>
    `;
    document.body.appendChild(panel);

    panel.querySelector('#yzOperatPrepaColumnsClose').addEventListener('click', () => { 
      panel.style.display = 'none'; 
    });
    
    panel.querySelector('#yzOperatPrepaColumnsReset').addEventListener('click', () => {
      const table = getTable();
      const headings = readHeadings(table);
      const body = panel.querySelector('#yzOperatPrepaColumnsBody');
    ...
    panel.querySelector('#yzOperatPrepaColumnsApply').addEventListener('click', () => {
      const table = getTable();
      const body = panel.querySelector('#yzOperatPrepaColumnsBody');
  ...
  window.toggleColumnVisibility = function() {
    const panel = document.getElementById('yzOperatPrepaColumnsPanel');

/**
 * ================== GESTION DES ARTICLES - MODULE GLOBAL ==================
 *
 * Module réutilisable pour la gestion des articles dans les commandes
 *
 * Fonctionnalités:
 * - Ajout/modification/suppression d'articles
 * - Gestion des variantes (taille, couleur)
 * - Système de prix upsell dynamique
 * - Protection des prix gelés
 * - Gestion des quantités
 * - Calcul automatique des totaux
 *
 * Utilisation:
 * 1. Inclure ce fichier dans votre template
 * 2. S'assurer que la commande a un attribut data-commande-id
 * 3. Les fonctions sont automatiquement disponibles globalement
 *
 * Dépendances:
 * - remise.js (pour la gestion des remises)
 * - Template: common/commande/section_articles.html
 * - Template: common/commande/_articles_section.html (partial)
 *
 * @version 3.0 - Version globale réutilisable
 * @author YZ-RESCUE
 */
console.log('✅ gestion_articles.js (GLOBAL) chargé - Version 3.0');
...
function getUrlModifier() {
    ...
    console.error('❌ window.urlModifier n\'est pas défini. Veuillez configurer cette variable dans votre template.');
    return null;
}
...
function ajouterNouvelArticle() {
    ...
    if (titleElement) {
        titleElement.textContent = 'Ajouter un article';
    }

    // Afficher le champ de sélection, cacher l'affichage (si ces éléments existent)
    const selectionField = document.getElementById('article-selection-field');
    if (selectionField) {
        selectionField.classList.remove('hidden');
    }

    const displayField = document.getElementById('article-display-field');
    if (displayField) {
...
function modifierArticle(panierId) {
    ...
    const titleElement = document.getElementById('articleModalTitle');
    if (titleElement) {
        titleElement.textContent = 'Modifier l\'article';
    }
    ...
    if (articleCard) {
        ...
        if (quantiteInput) {
            if (quantiteElement && quantiteElement.value) {
                quantiteInput.value = quantiteElement.value;
            } else {
...
function chargerArticlesDisponibles() {
    ...
    // URL de l'API - utiliser la variable window si définie, sinon fallback vers operateur-confirme
    const apiUrl = window.urlApiArticles || '/operateur-confirme/api/articles-disponibles/';
...
// Fonction pour filtrer les articles par type (promo, liquidation, test, upsell, all)
function filtrerArticles(type, event) {
    ...
    console.log(`🔍 Filtrage des articles par type: ${type}`);

    // Mettre à jour les badges de filtre (actif/inactif)
    document.querySelectorAll('.filter-badge').forEach(badge => {
        badge.classList.remove('active', 'ring-2', 'ring-blue-500', 'shadow-md');
    });

    const filterBadge = document.getElementById(`filter-${type}`);
    if (filterBadge) {
        filterBadge.classList.add('active', 'ring-2', 'ring-blue-500', 'shadow-md');
    }

    // Filtrer les lignes du tableau
    const rows = document.querySelectorAll('#articlesTableBody tr:not(.articles-spinner)');
    let visibleCount = 0;
    ...
    rows.forEach(row => {
        try {
            ...
            if (type === 'all') {
                ...
                shouldShow = article.has_promo_active === true;
            } else if (type === 'liquidation') {
                shouldShow = article.phase === 'LIQUIDATION';
            } else if (type === 'test') {
                shouldShow = article.phase === 'TEST';
            } else if (type === 'upsell') {
                shouldShow = article.isUpsell === true;
            }

            row.style.display = shouldShow ? '' : 'none';
            if (shouldShow) visibleCount++;
        } catch (e) {
            console.warn('Erreur lors du filtrage:', e);
            row.style.display = 'none';
        }
    });

    console.log(`✅ ${visibleCount} article(s) affiché(s) après filtrage`);
    ...
    if (visibleCount === 0) {
        ...
        if (tableBody && !tableBody.querySelector('.no-results-row')) {
            ...
            noResultsRow.innerHTML = '<td colspan="4" class="px-4 py-6 text-center text-gray-500">Aucun article trouvé pour ce filtre</td>';
...
function chargerVariantesArticle(article) {
    try {
        ...
        const baseUrl = window.urlGetVariants || '/operateur-confirme/get-article-variants/';
    ...
    valeurs1.forEach(val1 => {
        ...
        valeurs2.forEach(val2 => {
            ...
            const variant = variants.find(v => {
                const v1 = dimension1 === 'couleur' ? v.couleur : v.pointure;
                const v2 = dimension2 === 'couleur' ? v.couleur : v.pointure;
            ...
            if (variant) {
                ...
                tableHtml += `
                    <td class="px-4 py-3 text-center border-l border-gray-100 bg-white">
                        <div class="space-y-2">
                            <div class="text-lg font-bold text-green-600">${variant.prix || 0} DH</div>
...
function creerTableauSimple(variants, dimension, valeurs, variantMap) {
    ...
    valeurs.forEach(val => {
        const variant = variants.find(v => {
            const v1 = dimension === 'couleur' ? v.couleur : v.pointure;
            return v1 === val;
        });
        ...
        if (variant) {
            ...
            tableHtml += `
                <tr class="border-b border-gray-100">
                    ...
                    <td class="px-4 py-3 text-center text-lg font-bold text-green-600 bg-white">${variant.prix || 0} DH</td>
            ...
            details += `
                <div class="border-l-4 border-blue-200 pl-3 mb-2 ${index > 0 ? 'mt-3' : ''}">
                    ...
                    <div class="text-sm text-gray-600">
                        <strong>Prix:</strong> ${variant.prix || 0} DH | 
        ...
        window.selectedVariants.forEach(variant => {
            try {
                ...
                const articleAvecVariante = {
                    ...
                    prix_actuel: variant.prix || 0,
                    prix_unitaire: variant.prix || 0,
...
function rechercherArticlesPourVariantes(terme) {
    try {
        ...
        const baseUrl = window.urlApiArticles || '/operateur-confirme/api/articles-disponibles/';
        fetch(`${baseUrl}?q=${encodeURIComponent(terme)}`)
...
function initCustomScrollbar() {
    ...
    if (!container || !scrollbar || !thumb) {
        // Les éléments de scrollbar personnalisée sont optionnels
        return;
    }
...
function rafraichirSectionArticles() {
    ...
    const rafraichirUrl = window.urlRafraichir || `/operateur-confirme/api/commande/${(typeof window !== 'undefined' && window.commandeId) ? window.commandeId : ''}/rafraichir-articles/`;
    fetch(rafraichirUrl)
        .then(response => {
            if (!response.ok) {
                throw new Error(`HTTP ${response.status}: ${response.statusText}`);
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // chargerCommentairesDisponibles(); // TODO: Fonction non implémentée dans le module global

    // Charger les articles disponibles au chargement de la page
    chargerArticlesDisponibles();

    // Charger les opérations existantes depuis la base de données
    // chargerOperationsExistantes(); // TODO: Fonction non implémentée dans le module global

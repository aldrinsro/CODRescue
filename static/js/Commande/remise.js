/**
 * ================== GESTION DES REMISES - MODULE GLOBAL ==================
 *
 * Module reutilisable pour la gestion des remises sur les paniers
 *
 * Fonctionnalites:
 * - Application de remises personnalisees (pourcentage ou montant fixe)
 * - Previsualisation en temps reel
 * - Retrait de remises
 * - Mise a jour automatique de l'affichage
 * - Calcul automatique des totaux
 *
 * Types de remises supportes:
 * - POURCENTAGE: remise en %
 * - MONTANT_FIXE: remise en DH
 *
 * Utilisation:
 * 1. Inclure ce fichier dans votre template
 * 2. S'assurer qu'un modal avec id="remiseModal" existe
 * 3. Appeler ouvrirModalRemise(panierId) pour ouvrir le modal
 *
 * Dependances:
 * - gestion_articles.js (pour mettreAJourTotalCommande)
 * - Backend endpoint: calculer-remise-preview/{panier_id}/
 * - Backend endpoint: appliquer-remise/{panier_id}/
 * - Backend endpoint: retirer-remise/{panier_id}/
 *
 * @version 2.0 - Version globale reutilisable
 * @author YZ-RESCUE
 */
console.log('✅ remise.js (GLOBAL) charge - Version 2.0');
...
function calculerApercu() {
    ...
    const baseUrl = window.urlCalculerRemisePreview || '/operateur-confirme/calculer-remise-preview/';
    const url = `${baseUrl}${panierId}/?type_remise=${typeRemise}&valeur_remise=${valeurRemise}`;
...
function appliquerRemise() {
    ...
    // Appeler l'API
    const baseUrl = window.urlAppliquerRemise || '/operateur-confirme/appliquer-remise/';
    fetch(`${baseUrl}${panierId}/`, {
        method: 'POST',
        headers: {
...
function retirerRemise(panierId) {
    ...
    // Appeler l'API
    const baseUrl = window.urlRetirerRemise || '/operateur-confirme/retirer-remise/';
    fetch(`${baseUrl}${panierId}/`, {
        method: 'POST',
        headers: {
            'X-CSRFToken': csrfToken,
...
/**
 * Met à jour l'affichage de la remise dans l'interface
 * @param {number} panierId - ID du panier
 * @param {Object} data - Données de la remise
 */
function mettreAJourAffichageRemise(panierId, data) {
    console.log(`🔄 Mise à jour de l'affichage de la remise pour le panier ${panierId}`, data);

    const panierCard = document.querySelector(`[data-article-id="${panierId}"]`);
    if (!panierCard) {
        console.error(`❌ Carte du panier ${panierId} non trouvée`);
        return;
    }

    // 1. Ajouter le badge "Remise Appliquée" au titre de l'article si pas déjà présent
    ...
    if (articleTitle) {
        ...
        if (!remiseBadge) {
            ...
            remiseBadge.innerHTML = '<i class="fas fa-tag mr-1"></i>Remise Appliquée';
            articleTitle.appendChild(remiseBadge);
            console.log(`✅ Badge "Remise Appliquée" ajouté au titre de l'article ${panierId}`);
        }
    } else {
        console.warn(`⚠️ Titre de l'article non trouvé pour le panier ${panierId}`);
    }

    // 2. Mettre à jour le sous-total avec la couleur verte
    const sousTotalElement = document.getElementById(`sous-total-${panierId}`);
    if (sousTotalElement) {
        ...
        console.log(`✅ Sous-total mis à jour: ${data.nouveau_sous_total.toFixed(2)} DH (couleur verte)`);
    } else {
        console.warn(`⚠️ Élément sous-total non trouvé pour le panier ${panierId}`);
    }

    // 3. Chercher ou créer le conteneur de remise
    let remiseContainer = panierCard.querySelector('.remise-info-container');
    if (!remiseContainer) {
        // Créer le conteneur après le sous-total
        ...
        if (sousTotalDiv) {
            ...
            console.log(`✅ Conteneur de remise créé pour le panier ${panierId}`);
        } else {
            console.error(`❌ Parent du sous-total non trouvé pour le panier ${panierId}`);
            return;
        }
    }
    ...
    `;
    console.log(`✅ Informations de remise affichées dans le conteneur`);
    ...
    if (btnRemise) {
        ...
        console.log(`✅ Bouton "Appliquer remise" caché pour le panier ${panierId}`);
    }

    console.log(`✅ Affichage de la remise mis à jour avec succès pour le panier ${panierId}`);
}
...
function supprimerAffichageRemise(panierId) {
    console.log(`🗑️ Suppression de l'affichage de la remise pour le panier ${panierId}`);
    ...
    // 1. Supprimer le badge "Remise Appliquée" du titre de l'article
    const articleTitle = panierCard.querySelector('.article-title');
    if (articleTitle) {
        const remiseBadge = articleTitle.querySelector('.remise-appliquee-badge');
        if (remiseBadge) {
            remiseBadge.remove();
            console.log(`✅ Badge "Remise Appliquée" supprimé du titre de l'article ${panierId}`);
        }
    }

    // 2. Réinitialiser la couleur du sous-total à la couleur d'origine (#6d4b3b)
    const sousTotalElement = document.getElementById(`sous-total-${panierId}`);
    if (sousTotalElement) {
        sousTotalElement.style.color = '#6d4b3b'; // Couleur marron d'origine
        console.log(`✅ Couleur du sous-total réinitialisée pour le panier ${panierId}`);
    }
    ...
    if (remiseContainer) {
        ...
        console.log(`✅ Conteneur de remise supprimé pour le panier ${panierId}`);
    }

    // 4. Réafficher le bouton "Appliquer remise"
    const btnRemise = panierCard.querySelector('.btn-appliquer-remise');
    if (btnRemise) {
        btnRemise.style.display = ''; // Réafficher le bouton
        console.log(`✅ Bouton "Appliquer remise" réaffiché pour le panier ${panierId}`);
    }

    console.log(`✅ Affichage de la remise supprimé avec succès pour le panier ${panierId}`);
}

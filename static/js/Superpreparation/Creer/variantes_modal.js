...
class VariantesManager {
    ...
    async submitFormWithAjax(form) {
        try {
            ...
            if (this.variantes.length > 0) {
                ...
                const variantesResponse = await fetch('/Superpreparation/articles/variantes/creer-ajax/', {
                    method: 'POST',
                    body: JSON.stringify({
            ...
            setTimeout(() => {
                window.location.href = '/Superpreparation/articles/';
            }, 2000);
    ...
    async createVariantesForModification(articleId) {
        try {
            ...
            const response = await fetch('/Superpreparation/articles/variantes/creer-ajax/', {
                method: 'POST',
                body: JSON.stringify({
